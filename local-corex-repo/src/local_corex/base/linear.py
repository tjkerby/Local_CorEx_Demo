...
class LinearCorex(BaseCorex):
    ...
    """
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    THIS SEGMENT WAS ADDED BY THOMAS KERBY 12/18/23 FOR LOCAL COREX IN ORDER TO LOOK AT LATENT FACTORS
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    """
    
    def inverse_transform(self, x):
        inv_mat = np.linalg.pinv(self.ws.T)
        untrans_x = np.matmul(x, inv_mat)
        new_x = untrans_x*self.theta[1] + self.theta[0]
        return new_x
    
    def inv_transform(self, x):
        warnings.warn(
            "inv_transform is deprecated; use inverse_transform instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        return self.inverse_transform(x)
    
    def retransform(self, x): # Added by TK on 12/18/23
    ...
    """
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    THIS IS THE END OF THE SEGMENT THAT THOMAS KERBY EDITED ON 12/18/23
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    """
    ...
    def preprocess(self, x, fit=False):
        ...
        if self.gaussianize == 'none':
            ...
            if fit:
                ...
                self.theta = (mean, std)
            x = ((x - self.theta[0]) / self.theta[1])
            if np.abs(x).max() > 6 and self.verbose:
...
def pick_n_hidden(data, repeat=1, verbose=False, **kwargs):
    ...
    while True:
        ...
        for _ in range(repeat):
            out = LinearCorex(n_hidden=n, gpu=False, **kwargs).fit(data)
            m = out.moments
...
def standardize(x):
    ...
    std = np.std(x, axis=0, ddof=0).clip(1e-10)
    theta = (mean, std)
    std_x = g((x - theta[0]) /theta[1])
    return np.mean(std_x, axis = 0), theta

def destandardize(std_x, theta):
    return theta[1] * g_inv(std_x) + theta[0]
