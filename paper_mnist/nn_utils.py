...
def load_models(ae_ckpt: str, classifier_ckpt: str, ae_conf: dict, clf_conf: dict) -> LightningModule:
    ...
    # Load classifier
    classifier = MLPClassifier.load_from_checkpoint(classifier_ckpt, conf=clf_conf, map_location=torch.device('cpu'))
    
    # Load autoencoder checkpoint manually
    ae_checkpoint = torch.load(ae_ckpt, map_location=torch.device('cpu'))
    state_dict = ae_checkpoint['state_dict']
    
    # Infer encoder architecture from Linear layers only
    encoder_layers = []
    for key in sorted(state_dict.keys()):
        if key.startswith('encoder.') and key.endswith('.weight') and len(state_dict[key].shape) == 2:
            encoder_layers.append(state_dict[key].shape[0])
    
    # Infer decoder architecture from Linear layers only (excluding the final output layer)
    decoder_layers = []
    for key in sorted(state_dict.keys()):
        if key.startswith('decoder.') and key.endswith('.weight') and len(state_dict[key].shape) == 2:
            out_size = state_dict[key].shape[0]
            if out_size != 784:  # Exclude final output layer
                decoder_layers.append(out_size)
    
    # Check if batch norm is used
    use_batch_norm = any('running_mean' in key for key in state_dict.keys())
    
    # Infer if dropout was used by checking layer indices
    # If dropout was used, Linear layers appear at indices 0, 4, 8, ... (stride of 4)
    # If no dropout, Linear layers appear at indices 0, 3, 6, ... (stride of 3)
    encoder_linear_indices = [int(key.split('.')[1]) for key in state_dict.keys() 
                              if key.startswith('encoder.') and key.endswith('.weight') and len(state_dict[key].shape) == 2]
    if len(encoder_linear_indices) > 1:
        stride = encoder_linear_indices[1] - encoder_linear_indices[0]
        has_dropout = (stride == 4)  # Linear, BN, ReLU, Dropout
    else:
        has_dropout = False
    
    # Create configuration from inferred architecture
    inferred_ae_conf = {
        'encoder_layers': encoder_layers,
        'decoder_layers': decoder_layers,
        'use_batch_norm': use_batch_norm,
        'drop_out_p': 0.5 if has_dropout else 0.0,  # Common dropout value
        'lr': ae_conf.get('lr', 1e-3)
    }
    
    print(f"Inferred autoencoder config: {inferred_ae_conf}")
    
    # Create autoencoder with the inferred configuration
    autoencoder = Autoencoder(inferred_ae_conf)
